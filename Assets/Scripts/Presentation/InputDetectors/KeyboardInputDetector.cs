...
{
    // Input Action Reference を購読し、回転・移動・落下を委譲する
    public sealed class KeyboardInputDetector : MonoBehaviour
    {
        ...
        private void HandleRotate(CubeOperation operation)
        {
            ...
            if (_cubeUIController == null)
                throw new MissingReferenceException($"{nameof(KeyboardInputDetector)}: {nameof(_cubeUIController)} is not assigned.");
