...
## [Task 043] スコア・レベル表示の実装 [x]
- **ステータス**: 完了 ✅
- **優先度**: 中
- **概要**: フィールドエリアの上部にスコアとレベルを表示する UI を実装する。
- **実装対象**:
    - `ScoreUIView`: `MonoBehaviour`。`GameStateMachine.GameStateObservable` を R3 で購読し、スコア・レベル・消去ライン数が変化したときに表示を更新する
    - スコアは左上、レベルは右上に TextMeshPro で表示する
    - 表示形式: `SCORE: 1200`、`LEVEL: 2`、`LINES: 10`
    - `GameState.IsGameOver` が true になったら購読を解除する
- **配置**: `Assets/Scripts/Presentation/Views/Gameplay/ScoreUIView.cs`
- **完了条件**:
    - スコア・レベル・ライン数がリアルタイムで更新されること
    - ゲームオーバー時に更新が止まること

## [Task 044] スコア・レベル表示の動作確認 [ ]
- **ステータス**: 未着手
- **優先度**: 中
- **概要**: スコア・レベル表示が正しく動作することを Unity エディタで目視確認する。
- **確認内容**:
    - ライン消去時にスコアが正しく加算されること
    - 10ライン消去でレベルが上がること
    - ゲームオーバー時に表示が止まること
- **完了条件**: Unity エディタの Play モードで上記が目視確認できること

## [Task 045] テトリス操作ボタンの実装 [x]
- **ステータス**: 完了 ✅
- **優先度**: 高
- **概要**: スマホタッチ操作用のテトリス操作ボタン（左移動・右移動・ソフトドロップ・ハードドロップ）を実装する。フィールドエリア下部に Canvas で固定配置する。
- **実装対象**:
    - `TetrisInputView`: `MonoBehaviour`。各ボタンの入力を検知し `MoveMinoUseCase` / `DropMinoUseCase` → `GameStateMachine.ApplyGameState` で反映する
    - 左移動・右移動: `OnPointerDown` で移動開始、押し続けで一定間隔（0.1秒）で連続移動、`OnPointerUp` で停止
    - ソフトドロップ: `OnPointerDown` で押し続けている間、一定間隔（0.05秒）でソフトドロップを繰り返す
    - ハードドロップ: `OnPointerDown` で1回だけ実行
    - スクランブル中・ゲームオーバー時は入力を無視する
    - `Initialize(GameStateMachine stateMachine)` で `_stateMachine` を受け取る
- **配置**: `Assets/Scripts/Presentation/Views/Gameplay/TetrisInputView.cs`
- **UI配置**:
    - Canvas（Screen Space Overlay）のフィールドエリア下部に固定
    - 左側: ← ボタン・→ ボタン
    - 右側: ↓ ボタン・⬇⬇ ボタン
- **完了条件**:
    - タッチで左右移動・ソフトドロップ・ハードドロップが動作すること
    - 押し続けで連続移動・連続ソフトドロップが動作すること

## [Task 046] テトリス操作ボタンの動作確認 [ ]
- **ステータス**: 未着手
- **優先度**: 高
- **概要**: テトリス操作ボタンが正しく動作することを確認する。
- **確認内容**:
    - 左右ボタンで移動できること
    - 押し続けで連続移動できること
    - ソフトドロップで加速落下できること
